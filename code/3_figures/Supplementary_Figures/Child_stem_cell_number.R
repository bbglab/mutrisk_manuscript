...
# TODO: Remove all code which is not used for further scripts
library(ggh4x)
...
plot_saturation_age = function(intersects_ci, analysis_name, groupby) {
  get_saturation(intersects = intersects_ci, groupby) |>
    select(-ncells) |>
    pivot_wider(values_from = "prob", names_from = "name") |>
...
analyze_probability = function(gene_counts, analysis_name, groupby = "donor",
  ...
  for (tissue in c("colon", "lung", "blood")) {
    ...
    for (category_select in unique(expected_rates$category)) {
      ...
      for (name in names(expected_rates_list)) {
        ...
        ncells_all <- metadata_child_cell_numbers_ci |>
          filter(donor %in% !!name)  |> distinct()
        ncells = ncells_all |> filter(name == "mid_estimate") |> pull(ncells)
        nells_ci = ncells_all |> pull(ncells)
        ncells_ci[is.na(ncells_ci)] = ncells # for kids, remove the high and low confidence intervals
        print(name)
        ...
        list_intersects_ci[[name]] = get_prob_mutated_range(expected_rates_sample = expected_rates_list[[name]],
                                                            gene_counts = gene_site_counts,
                                                            ratios_cat = ratios_cat, range = nells_ci) |>
  ...
  plot_list[["plot_saturation_age"]] = plot_saturation_age(intersects_ci |> filter(name == "mid_estimate") |> distinct(), analysis_name, groupby)
  plot_list[["plot_saturation_age_ci"]] = plot_saturation_age_ci(intersects_ci |>  distinct(), analysis_name, groupby)
...
A = readRDS("manuscript/figure_panels/figure_2/figure_2C.rds")
# add childhood number lines to the curve-plot
childhood_ncells = tissue_ncells_ci_age |>
  filter(age_category == "child") |>
  dplyr::select(tissue, mid_estimate) |>
  filter(tissue != "blood")

childhood_cell_n_df = childhood_ncells |>
  mutate(label = paste0("10-year old - ", format_bignum(mid_estimate)))

childhood_probabilities = exome_analysis_normal$plot_list$plot_saturation_curve_ci +
  geom_segment(data = childhood_cell_n_df, aes(x = mid_estimate, xend =  mid_estimate, y = 0, yend = 1),
